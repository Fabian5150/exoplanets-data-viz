...
Here we have our first filtration: 
We used planet radius and mass a basis for classification:
- Earth-like:
...
# Plots from metoring 2 - basic understanding the data 
...
# Load dataset
data <- read.csv("C:/Users/krali/Downloads/nasa_aggregated.csv")

# Define planet types based on radius, mass, and temperature
#should be done in preprocesing mby?
...
classify_planet <- function(radius, mass, temp) {
  if (is.na(radius) | is.na(mass) | is.na(temp)) {
    ...
    return("Earth-like")
  } else if (radius > 1.5 & radius <= 2.5 & mass > 5 & mass <= 10 & temp > 370 & temp <= 600) {
    return("Super-Earth")
  } else if (radius > 2.5 & radius <= 4 & mass > 10 & mass <= 50 & temp > 600 & temp <= 1000) {
    return("Mini-Neptune")
  } else if (radius > 4 & radius <= 6 & mass > 50 & mass <= 100 & temp > 1000 & temp <= 1500) {
    return("Neptune-like")
  } else if (radius > 6 & mass > 100 & temp > 1500) {
    return("Gas Giant")
  } else {
...
# 1. Scatter Plot: Mass vs. Radius with filtred data so it is visible
...
ggplot(filtered_data, aes(x = pl_rade, y = pl_bmasse, color = Planet_Type)) +
  ...
  scale_color_manual(values = c("Earth-like" = "green", "Super-Earth" = "blue", "Mini-Neptune" = "purple", "Neptune-like" = "orange", "Gas Giant" = "red", "Unknown" = "gray")) +
...
filtered_data <- data %>% filter(pl_rade < 10 & Planet_Type != "Unknown")
...
# 3. Boxplot: Temperature Distributions by Planet Type
ggplot(data, aes(x = Planet_Type, y = pl_eqt, fill = Planet_Type)) +
  geom_boxplot() +
  scale_fill_manual(values = c("Earth-like" = "green", "Super-Earth" = "blue", "Mini-Neptune" = "purple", "Neptune-like" = "orange", "Gas Giant" = "red", "Unknown" = "gray")) +
  labs(title = "Equilibrium Temperature by Planet Type", x = "Planet Type", y = "Equilibrium Temperature (K)") +
  theme_minimal()


# 5. Bar Chart: Number of Exoplanets by Type
...
# 6. Bar Chart: Number of Exoplanets by Type - unknown excluded
planet_counts <- data %>%
...
ggplot(planet_counts, aes(x = Planet_Type, y = count, fill = Planet_Type)) +
  geom_bar(stat = "identity") +
  scale_fill_manual(values = c("Earth-like" = "green", "Super-Earth" = "blue", "Mini-Neptune" = "purple", "Neptune-like" = "orange", "Gas Giant" = "red")) +
  labs(title = "Number of Exoplanets by Type", x = "Planet Type", y = "Count") +
  theme_minimal()
...
earth_like_planets <- data %>%filter(Planet_Type == "Earth-like")
ggplot(earth_like_planets, aes(x = pl_rade, y = pl_bmasse, size = pl_eqt)) +
  geom_point(alpha = 0.7) +
  labs(title = "Bubble Chart: Radius vs. Mass (Size by Temperature)",
       x = "Planet Radius (Re)",
       y = "Planet Mass (Me)",
       size = "Equilibrium Temperature (K)") +
...
# Plot the bar chart without x-axis labels
...
ggplot(filtered_data, aes(x = sy_dist, y = pl_bmasse, size = pl_rade, color = Planet_Type)) +
  geom_point(alpha = 0.6) +
  scale_size_continuous(range = c(2, 8)) +
  scale_color_manual(values = c("Earth-like" = "green", "Super-Earth" = "blue", "Mini-Neptune" = "purple", "Neptune-like" = "orange", "Gas Giant" = "red")) +
...
```
# Understanding the planets that are earth-like by looking at the data of the 4 planets
...
# Interactive stuff part one 
- kinda working doesnt give an error when you add earth
...
ui <- fluidPage(
  ...
  sidebarLayout(
    sidebarPanel(
      ...
      actionButton("add_earth", "Add Earth for Comparision")  # Earth comparison button
    ),
    mainPanel(
...
server <- function(input, output, session) {
  ...
  earth_added <- reactiveVal(FALSE)  # Flag to check if Earth is added
  
  # Observe event when category changes to update available planet options
  ...
  # Update filtered data based on selected planets and category
  ...
  output$radar_chart <- renderPlotly({
    # Change to plot stellar radius instead of orbital period
    df <- filtered_data() %>% select(pl_name, st_rad, pl_orbsmax, pl_rade, pl_bmasse)
    df_long <- tidyr::pivot_longer(df, -pl_name, names_to = "Variable", values_to = "Value")
    
    # Add Earth data if button is pressed
    if (earth_added()) {
      earth_long <- data.frame(
        pl_name = "Earth", 
        Variable = c("st_rad", "pl_orbsmax", "pl_rade", "pl_bmasse"),  # Stellar radius included
        Value = c(1, 1, 1, 1)  # Earth values for each variable
      )
      df_long <- rbind(df_long, earth_long)
    }

    plot_ly(df_long, r = ~Value, theta = ~Variable, color = ~pl_name, type = "scatterpolar", fill = "toself") %>%
      layout(
  ...
  output$bar_chart <- renderPlotly({
    df <- filtered_data()
    plot_ly(df, x = ~pl_name, y = ~pl_masse, type = "bar", color = ~pl_name) %>%
      layout(title = "Exoplanet Mass Comparison", yaxis = list(title = "Mass (Earth Masses)"))
  })
  
  # Observe the Earth button click and set the flag
  observeEvent(input$add_earth, {
    earth_added(!earth_added())  # Toggle the Earth flag when the button is clicked
  })
}
...
# Interactive window 
only problem with one plot if you add earth to it
```{r}
library(shiny)
library(plotly)
library(dplyr)
...
server <- function(input, output, session) {
  ...
  output$temperature_chart <- renderPlotly({
    ...
    plot_ly(df, x = ~pl_name, y = ~pl_eqt, type = "bar", color = ~pl_name) %>%
      layout(title = "🌡️ Equilibrium Temperature Comparison", yaxis = list(title = "Temperature (K)"))
  })
